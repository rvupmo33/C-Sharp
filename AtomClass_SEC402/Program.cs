...
class Program
{
    static void Main(string[] args)
    {
        AddAllElements();
        DisplayAllElements();
        serializeFirstItem("FirstElement.json", elements);
        readFirstItem("FirstElement.json");
        serializeAllItem("AllElements.json", elements);
        readAllItems("AllElements.json");
    }
    static List<Atom> elements = new List<Atom>();
    
    public static void DisplayAllElements()
    {
        Console.WriteLine("All elements in the collection elements:");
    ...
    public static void serializeFirstItem(string fileNameOne, List<Atom> ListOfElements)
    {
        StreamWriter writer = new StreamWriter(fileNameOne);

        string jsonString = JsonSerializer.Serialize(ListOfElements[0]);
        writer.Write(jsonString);
        Console.WriteLine("Element being written into 'FirstElement.json'");
    ...
    public static void readFirstItem(string fileNameOne)
    {
        List<Atom> list = new List<Atom>();
        StreamReader reader = new StreamReader(fileNameOne);
        string line = reader.ReadLine();
        Console.WriteLine("Element being read from 'FirstElement.json'");
    ...
    public static void serializeAllItem(string fileNameTwo, List<Atom> ListOfElements)
    {
        StreamWriter writer = new StreamWriter(fileNameTwo);
        Console.WriteLine("Elements being written into 'AllElements.json'");
        foreach (Atom atom in ListOfElements)
        {
            string jsonString = JsonSerializer.Serialize(atom);
    ...
    public static void readAllItems(string fileNameOne)
    {
        List<Atom> list = new List<Atom>();
        StreamReader reader = new StreamReader(fileNameOne);
        string line = reader.ReadLine();
        Console.WriteLine("Elements being read from 'AllElements.json'");
        while (line != null)
        {
            Atom elm = JsonSerializer.Deserialize<Atom>(line); 
            list.Add(elm);
